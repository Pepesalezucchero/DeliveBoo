<section>
    <header>
        <h2 class="text-lg font-medium text-gray-900">
            {{ __('Modifica la tua Password') }}
        </h2>

        <p class="mt-1 text-sm text-gray-600">
            {{ __('Modifica la tua Password, si consiglia di usare combinazioni di lettere e numeri per sicurezza.') }}
        </p>
    </header>
    ...
    <form method="post" action="{{ route('password.update') }}" class="mt-6 space-y-6">
        ...
        <div class="d-flex align-items-center gap-4">
            ...
            <p id='status' class=" fs-5 text-muted">{{ __('Informazioni salvate.') }}</p>
            @endif
        </div>
    </form>
</section>
